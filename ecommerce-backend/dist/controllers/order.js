...
export const getSingleOrder = TryCatch(async (req, res, next) => {
    ...
    if (myCache.has(key))
        ...
        if (!order)
            return next(new ErrorHandler("order not found", 404)); /* The populate() method is used to replace the specified path (user) in the order documents with documents from a referenced collection (likely the users collection).*/
        myCache.set(key, JSON.stringify(order));
    }
    return res.status(200).json({
        success: true,
